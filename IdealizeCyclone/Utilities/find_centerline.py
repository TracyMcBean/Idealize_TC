...
filename = "../Data/center_fiona_idealized" # Name of file where center array should be saved
...
p_env = np.empty([len(pres_ds.time),len(pres_ds.height)-lev_start,1])
for t in range(0,len(pres_ds.time)):
    p_env[t,:,0] = [np.mean(pres_ds.pres[0,i].values) for i in range(lev_start, lev_end)]
   
center = np.empty([len(pres_ds.time),len(pres_ds.height)-lev_start,2])
...
print('This is the time len: %s' % len(pres_ds.time))

for t in range(33,len(pres_ds.time)):
    # The center must be found on each level
    for l in range(lev_start,lev_end):
        print( 'Finding center on lev:', l)
    
        # 1. First guess:
        if coord_unit=='rad':
            single_lev = pres_ds.pres.isel(time = t, height=l)
            ...
            center[l-lev_start,] = get_clonlat([clon_fg, clat_fg], single_lev, p_env[t, l], r_rad)
        
        if coord_unit=='deg':
            single_lev = pres_ds.isel(time = t, height=l)
            ...
            print('These are the shapes:', center[t].shape, p_env[t].shape)
            center[t,l-lev_start,] = get_lonlat([lon_fg, lat_fg], single_lev, p_env[t, l-lev_start], r_deg)
   
    np.save('../Data/tmp_center',center)
    print(center[t])
    print('Next time step:', t+1)
